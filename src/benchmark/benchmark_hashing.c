...
static void run_linear_probing(const int* keys, int m, int n)
{
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        while (table[hash_loc] != 0)
        {
            ...
            if (hash_loc == start) break; // Table full
        }
        if (table[hash_loc] == 0)
        {
            table[hash_loc] = val;
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        while (table[hash_loc] != 0)
        {
            if (table[hash_loc] == val) break;
            hash_loc = (hash_loc + 1) % n;
            if (hash_loc == start) break;
        }
    }
...
void run_hashing_benchmark(int n)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        switch (i)
        {
            case 0: run_linear_probing(keys, m, n); break;
            case 1: run_quadratic_probing(keys, m, n); break;
            case 2: run_double_hashing(keys, m, n); break;
            case 3: run_separate_chaining(keys, m, n); break;
        }
